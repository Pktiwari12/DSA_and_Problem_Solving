...
public:
    long long countPairs(int n, vector<vector<int>>& edges) {
        ...
        // Initially all noeds is assumed disconnectd.
        // so no. of pair out of n is equal to :- nC2
        long long result = static_cast<long>(n)*static_cast<long>((n-1))/2;
        for(int i = 0 ; i < n ; i++){
            result -= static_cast<long>(noOfElements[i]) * static_cast<long>(noOfElements[i]-1) / 2;
        }
        return result;
...
// Following code give time limit exceed
